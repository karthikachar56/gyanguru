        ...
        <div className="mt-6 space-y-4">
          <div className="relative max-w-xl">
            ...
            <input
              ...
              className="w-full pl-11 pr-4 py-3 rounded-xl bg-dark-900/90 border border-slate-700/80 text-white placeholder-slate-400 text-xs sm:text-sm focus:outline-none focus:border-brand-500 focus:ring-1 focus:ring-brand-500 transition-all shadow-inner"
            />
          </div>
              ...
              <button
                ...
                className={`px-3 py-1.5 rounded-xl text-xs font-semibold transition-all duration-200 ${
                  selectedRegion === r.id
                    ...
                    : 'bg-dark-900/80 text-slate-400 hover:text-white border border-slate-800 hover:border-slate-700'
                }`}
              >
                {r.label}
              </button>
      ...
      <div className="space-y-4">
        ...
        {filteredPortals.length === 0 ? (
          <div className="glass-panel p-10 text-center rounded-2xl border border-slate-800 space-y-2">
            ...
            <p className="text-sm font-semibold text-slate-300">No portal matching "{searchTerm}"</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {filteredPortals.map(portal => {
              ...
              return (
                <div
                  key={portal.id}
                  className="group relative flex flex-col justify-between glass-card rounded-2xl p-5 border border-slate-800 hover:border-brand-500/50 transition-all duration-300 hover:-translate-y-1 hover:shadow-xl bg-dark-900/60"
                >
                  <div>
                    {/* Top Bar */}
                    <div className="flex items-center justify-between gap-2 mb-3">
                      <div className="flex items-center space-x-2">
                        <span className="px-2.5 py-0.5 rounded-lg text-[11px] font-extrabold bg-brand-500/10 text-brand-300 border border-brand-500/30">
                          {portal.code}
                        </span>
                      ...
                      <span className="text-[10px] px-2 py-0.5 rounded-full bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 font-medium flex items-center gap-1">
                        <ShieldCheck className="w-3 h-3" /> Live Check Enabled
                      </span>
                    </div>
                    ...
                    {/* Exam Scope */}
                    <div className="mt-2 text-[11px] text-slate-400 flex items-start gap-1.5 leading-relaxed bg-dark-900/90 p-2.5 rounded-xl border border-slate-800/80">
                      <Award className="w-3.5 h-3.5 text-brand-400 shrink-0 mt-0.5" />
                      <span>{portal.exam}</span>
                    </div>

                    {/* Official Link Display */}
                    <div className="mt-3 text-[11px] font-mono text-slate-400 truncate flex items-center justify-between bg-dark-950 p-2 rounded-lg border border-slate-800">
